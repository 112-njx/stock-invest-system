...
> 目标：以「Python + FastAPI + PostgreSQL + Vue3 + Redis + Celery + Nginx + DeepSeek + 本地记忆」构建生产级量化交易软件。
> 参考开源：RAGFlow（RAG/Agent）、QuantDinger（AI 量化 Agent，J/K/L 区架构可完全借鉴）。
...
- **测试保障**：pytest 单测覆盖核心逻辑（指标计算、策略解析、记忆抽取），CI 强制跑测。
...
- **Agent 工具注册机制**：tool registry 插件化，新增分析能力不改核心循环（借鉴 QuantDinger 工具集）。
- **RAG 知识库扩展**：借鉴 RAGFlow，支持追加财报、公告等文档类型入知识库。
