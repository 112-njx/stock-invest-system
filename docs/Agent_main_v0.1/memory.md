...
20. V0.3 泳道 C（数据·AI 成本）五步全部完成（2026-09-17 ~ 09-18）：主序列 **G21 → G31**，穿插 **G34 / G15 / G14**，对应 P1-12b → P1-12c → P0-3b → P0-3a → P0-2。
    - **G21（P1-12b）记忆向量去 Chroma 迁 pgvector**：`store.py` 由 ChromaDB PersistentClient 重写为 SQLAlchemy + pgvector —— `memory_chunks` 行级增删改查取代 per-user collection，检索全链路下推 SQL（子查询按 `<=>` 余弦距离取候选 `max(k, min(3k,30))` → 外层按加权 score 排序取 TopK）；`find_duplicate` 的 0.85 去重阈值下推为 SQL；以 `user_id + embedding_kind` 行级过滤替代 collection 名隔离。`embedding.py` 去除 `chromadb.api.types` 依赖，改项目自有 `EmbeddingFunction` 协议。`memory_service` 由「store 写 Chroma + agent_repo 写 PG」双写合并为 store 一次落库。
    - **G31（P1-12c）存量回填 + 验证 + 下线**：回填脚本按 `(user_id, vector_id)` 左连接、幂等可重跑；**TopK 召回对比 4/4 一致、0 差异**；双写灰度开关 `MEMORY_DUAL_WRITE` 可开可关（2 项测试验证）。验证通过后整体下线：删除 `chroma_legacy.py` / 迁移脚本 / 双写测试 / `data/chroma/`，`pyproject` 与 `requirements.lock` 移除 chromadb，`config` 移除 `CHROMA_DIR` / `MEMORY_DUAL_WRITE` / `BACKUP_CHROMA`，`backup_service` 文件备份只留 memory/exports（向量随 pg_dump）。**硬证据：venv 内 `pip uninstall chromadb` 后全库 483 passed**。
    - **G34（P0-3b）memory_chunks.content 加密**：用 SQLAlchemy `TypeDecorator`（`models/types.py::EncryptedText`）在 ORM 边界统一加解密，而非各读写点手工处理 —— 记忆管理 API / 数据导出 / 重嵌入脚本 / 后台任务**所有读取方零改动**即拿到明文，不会漏出口把密文吐给用户。密文以 base64 文本落库，列类型仍 Text **无需迁移**；embedding 保持明文向量（加密后无法算相似度）；存量明文行按「非密文头则原样返回」兼容。
    - **G15（P0-3a）记忆文件加密 + 审计 + 存储说明**：新增 `core/crypto.py`（AES-256-GCM，`ENC1` 魔数 + 12B nonce + tag；密钥取 `MEMORY_ENCRYPTION_KEY`，长度不符直接报错，**未配置抛 `EncryptionKeyMissing` 不降级明文**，代码内无硬编码兜底）。记忆文件改为加密落盘（追加 = 解密→拼接→重加密），读取自动解密且兼容存量明文 .md。Alembic **0017** 建 `audit_log`（user_id FK CASCADE / action / memory_id / ip / created_at + `(user_id, created_at DESC)` 索引），记忆读/写/删/清空全部记审计（best-effort 不阻断主链路），新增 `GET /api/v1/memory/audit`。前端 `MemoryFilesDialog` 加「数据存储说明」提示条（仅加提示未改结构），`PrivacyView` 移除「本地存储设计」表述、改为服务端加密存储 + 补审计说明。
    - **G14（P0-2 简化版）用户自填 API Key + token 用量**：Alembic **0018** 给 `users` 加 `api_key_encrypted`（复用 G34 的 EncryptedText）/ `llm_tokens_prompt` / `llm_tokens_completion`。新增 `services/llm/user_key.py`：`sk-` 前缀形态校验、ORM 自动解密读取、原子自增累计、`build_llm_service_for_user`（用户 key 优先 + 用量回调走独立 SessionLocal）。`LLMService` 增 `with_api_key`，派生实例**共享进程级熔断器与令牌桶**（用谁的 key 不影响服务端防护）；`DeepSeekProvider` 开 `stream_usage=True` 使流式也能取到 usage，缺失时用 `token_budget.estimate_tokens` 估算并标记 `estimated`。端点 `GET/PUT /users/me/api-key`（**只回掩码不回明文**，传空串清除）；前端 `SettingsPanel` 加「AI 模型设置」区块。
    - **关键坑（务必记住）**：
      ① **召回口径等价性是迁移可比的前提** —— 旧 `_weighted_score` 写 `1 - distance/2`，Chroma 默认 `hnsw:space=l2` 返回**平方 L2 距离**，向量已 L2 归一（L2² = 2 - 2cos），故该式化简即 `cos`。新式 `1 - 余弦距离` 与之数学等价 —— 已用真实数据实测验证（chroma distance == 平方 L2，误差 <1e-5）。**换向量库前必须先实测旧口径，不能照字面假设**。
      ② **chromadb 1.5.x 的 query 路径要求 `embed_query()`**（旧版由 `EmbeddingFunction` 基类默认实现提供）—— 去掉继承后 `__call__` 不够用，会 `AttributeError`。迁移期需在遗留模块内适配（chromadb 按**签名结构**校验 EF，非 isinstance，故 `__call__(self, input)` 参数名必须一致）。
      ③ **`Path.write_text()` 在 Windows 会把 `
` 翻译成 `
`** —— 向 LF 文档追加内容会变成整文件行尾翻转，`git diff` 显示全文重写。追加文档应 `read_bytes`/`write_bytes` 并显式控制行尾；`fixed.md` 的 HEAD blob 本身是 CRLF（与 `core.autocrlf=true` 不一致），任何触碰都会全文 diff。
      ④ **多泳道共享文件提交必须逐段核对归属** —— 本轮 `config.py` 含泳道 F 未提交的 `BACKUP_*`、`Agent_code.md` 含泳道 F 的 G05 条目。做法：备份工作区 → `git checkout HEAD -- <doc>` → 只追加本泳道条目 → `git add` → 还原备份；`config.py` 用 `git hash-object -w` + `update-index --cacheinfo` 直接暂存「HEAD + 本泳道改动」的 blob。**切勿 `git add` 整个共享文件**。
      ⑤ **改「取服务实例」的代码路径会打断既有测试注入** —— G14 把 `stream_chat` 的装配改为内部调用单例，绕过了测试 monkeypatch 的 `chat_service.get_llm_service` 符号，导致 `test_chat` 降级用例失败。修法：装配函数增 `base` 参数由调用方传入单例，保留注入缝隙。
      ⑥ **全库 flaky 的定性方法** —— 本轮遇到「全库 24 failed / 1228s」但单文件全绿。用 `git stash push -- <本泳道文件>` 取基线对比，并观察失败集是否每次不同：若失败用例每次都不一样且单独跑全绿，即为环境/顺序性 flaky（网络依赖的行情源用例 + 共享 DB 顺序干扰），非本泳道回归。1228s 的异常耗时是网络降级时行情源超时重试所致。
      ⑦ **dev 全栈会被其他泳道停掉** —— 会话期间 db/redis 容器被停过两次，需 `docker compose --env-file .env.docker -f deploy/docker-compose.dev.yml up -d db redis` 恢复；后端容器因**镜像未重建、未装 pgvector** 而崩溃重启（宿主 venv 不受影响）。
    - **需人工配置**（详见 project_constraints_v0.3.md 第八节第 20/21 条）：**`MEMORY_ENCRYPTION_KEY`**（32 字节随机，生产必须配置；**该密钥一旦用于加密便不可更换**，更换后既有密文不可解密，需纳入密钥管理与备份范围；开发环境已生成随机值写入 gitignore 的 `.env`）；**重建后端镜像**（`docker compose ... build api worker beat`，否则容器内缺 pgvector）。
    - 验收：新增 **32 项**测试（G21/G31 共 13 项 + G15/G34 13 项 + G14 19 项，其中部分为改造既有用例）；全库 **528 passed / 0 failed**；前端 `vue-tsc -b --noEmit` 通过；Alembic 0017 / 0018 实跑落库成功。

20. V0.3 泳道 G（前端集成）三步全部完成（2026-09-18）：主序列 **G09 → G27**，穿插 **G35**。对应 P1-6a → P1-6b → 登录页改造（方案 B）。
    - **G09（P1-6a）列表分页 + 消息游标分页**：新增 `app/schemas/pagination.py`（`PageParams` + `page_envelope` `{items,total,page,size,total_pages}` + `cursor_envelope` `{items,has_more,next_cursor}`）。`GET /conversations`、`/strategies`、`/agents`、`/backtest/tasks` 由**裸数组**改统一信封；`/agent/runs` 原已分页、补齐 `total_pages`。`GET /conversations/{id}/messages` 改游标分页（默认最新 50 条 / `before` 往前翻 / 游标非法 400·40005）。**关键坑**：游标过滤不能用行值比较 `(created_at,id) < (?,?)`（SQLite 测试库不支持），必须写「`created_at < ? OR (created_at = ? AND id < ?)`」双分支；返回前 reverse 保证对外仍升序（前端渲染顺序不变）。仓储层 `list_*` 的 offset/limit 默认 None，内部全量调用方（`chat_service` 组装 LLM 上下文、`backtest_service` 取全部策略 id）行为不变。前端同一步适配（禁止只改一端）：`api/ai.ts` 加 `Page<T>`/`MessagePage`，`stores/ai.ts` 取 `.items` + 记录 total/游标。
    - **G27（P1-6b）虚拟滚动 + 消息增量加载 + 渲染埋点**：引入 `vue-virtual-scroller@3.0.5`（peer vue ^3.3.0）。新增 `composables/useInfiniteList.ts`（RecycleScroller `@update` 可见区间近末尾即续拉 + 渲染耗时测量）与 `utils/monitor.ts::trackRender`（`SLOW_RENDER_MS=500`，超阈值 `console.warn` + 走既有 monitor 通道上报 `render:<name>`）。SessionSidebar 会话/策略列表、AgentRunsDialog 改 RecycleScroller（**item 高度与 item-size 必须严格一致**，否则行错位：`.j-item` 固定 34px / `.run-item` 76px+8px margin）。AgentRunsDialog 的「上一页/下一页」分页器**已移除**，改为连续滚动 + 「已加载 X / 共 Y 条」。ChatMessages 自动滚底改监听**末条消息 id**而非 `messages.length`（length 变化在 prepend 更早消息时同样触发，会把用户拽回底部）；滚动到顶部 48px 内增量加载并按新增高度差补偿 `scrollTop`。新增 `npm run verify:pagination`（esbuild + jsdom，25 项断言）。
    - **G35（登录页改造 · 方案 B）**：路由 `/` 改行情首页（`market-home`），`/market` 保留重定向，`/market/detail`、`/ai` 免登录，`/login` 独立保留。**RouteMeta 语义拆解**：原 `meta.public` 同时表示「免登录」与「不显示 AppBar」，改为 `meta.bare`（仅登录/找回密码/法律页等独立页不显示导航与公告条）；守卫简化为唯一方向性跳转（已登录访问 `/login` 回首页）。新增 `components/auth/AuthForm.vue`（独立登录页与登录弹窗**复用同一份表单**，G33 邮箱必填 / G03 协议勾选 / G18 账户恢复全保留）、`stores/authModal.ts` + `components/auth/LoginModal.vue`、`components/layout/UserMenu.vue`（未登录=登录入口；已登录=下拉菜单 个人设置/我的数据/通知中心/退出登录，与 G16 铃铛并存）。视觉按 `login_page_design_v0.3.md` 重写 LoginView（左 56/右 44、深色渐变、无 logo、三卡片可配置 + 鼠标重心 3D 倾斜 `composables/useTilt.ts`）。新增 `npm run verify:routing`（20 项断言）。
    - **G35 最重要的坑（后续务必遵守）**：**未登录状态下任何鉴权请求都不能发出** —— `api/http.ts` 的 401 拦截器在 refresh 失败时会 `clearAuth()` 并跳登录页，会把免登录访客**直接弹走**。已为 `/watchlist`、支撑压力位、通知未读数、API Key 状态、会话/策略/Agent 列表全部加未登录短路；**新增鉴权调用时请同样先判 `user.token`**。
    - **G27 虚拟滚动第二坑**：jsdom 不做布局计算（`clientHeight` 恒为 0），RecycleScroller 在无布局环境下无法真实渲染，**虚拟滚动的 DOM 行为无法自动化验证**，只能靠 typecheck/build + 人工浏览器确认。
    - 验收：G09 新增 11 项 + G27 脚本 25 项 + G35 脚本 20 项断言全绿；全库 **562 passed / 2 skipped**（2 项 skip 属泳道 F G08 的 Windows `resource` 模块限制，带明确理由，非掩盖失败）；`vue-tsc` + `eslint` + `build` 全通过；`verify:xss` 无回归。
    - 需人工配置/确认（详见 project_constraints_v0.3.md 第八节第 22~28 条）：重新 `npm install`（新增 vue-virtual-scroller）、登录页三张卡片图 **5.2MB 未压缩需优化**、Agent 运行记录弹窗分页器改连续滚动的 UX 变更、方案 B 行为变更与浏览器验收清单。
    - 跨泳道提示：G09/G27/G35 三次提交均**未扫入他人未提交改动**（`project_constraints_v0.3.md` 中泳道 F 对第 20/22 条的在途修正已用「还原 HEAD → 追加本人条目 → 提交 → 恢复工作区」的外科方式规避，留在工作区由泳道 F 自行提交）。
