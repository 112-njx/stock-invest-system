...
| 检查点 | 异常情况 | 可能的异常原因 |
|---|---|---|
| SSE 流式对话 | 普通对话流式输出正常，markdown 渲染正常，无异常 | —（正常） |
| Token 用量展示 | 底部状态栏"本次对话已用 1,303 tokens"正常显示 | —（正常） |
| 会话标题自动生成 | J区会话标题从"新会话"自动更新为AI生成标题（截断显示） | —（正常） |
| 记忆文件面板 | 面板正常：0条统计、全部/高/中/低筛选、清空按钮、"暂无记忆"空态均存在 | —（正常） |
