...
首先阅读D:\stock-invest-system\docs\Agent\memory.md中的记忆文件，确保你可以迅速上手项目。观看D:\stock-invest-system\docs\project_docs\docs.md
  ，它是项目的详细需求文档，偏前端描述。其次你看到文件夹Agent_frontend，其中的md文件说明了他们的作用，假如你未来你将要在其中补充文字。注意不要修改这两个
  文件夹的md文件中我的说明文字，在此基础上添加即可。你的职责是该项目的前端开发工程师，你需要按照前端开发规划（开发规划你给出）一步一步按阶段下的细分任务完成开发。
  其中D:\stock-invest-system\docs\Agent_frontend\PageDesign.md是前端的页面描述。 
  D:\stock-invest-system\docs\Agent_backend\api-docs.md是后端api文档，在对接api时你需要对照该文档，你不要更改或删除该文档最上方的文字说明。
  如果你对具体的开发细节不清楚，请停止工作，先问我你疑惑的开发细节，清楚后在进行开发。另外开发的前
  ...
  在你编程完一个阶段下的一个细分任务后，在D:\stock-invest-system\docs\Agent_frontend\Agent_code.md中按文件要求补充编码记录，同样不要更改
  文档最上方的文字说明。你在开发完成后需要在D:\stock-invest-system\docs\Agent\memory.md中按照格式对你的开发进行总结，阶段进行同步，方便其他agent阅读
  你清楚之后，完成前端任务的开发。开发过程中请你用中文回答我。
