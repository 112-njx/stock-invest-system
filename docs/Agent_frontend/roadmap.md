...
### 第二波（前端阶段四 AI流式稳定性 + 阶段五 记忆可视化，2026-08-23 完成）
- SSE 断线续传/错误分级/降级横幅均在现有 AI 页生效，无需新配置；降级模式观察需临时去掉后端 .env 的 DEEPSEEK_API_KEY 后重启后端再对话，可见蓝色横幅+基础分析文案（验证后恢复）。
- 5.1 M区「记忆文件」面板（查看/删除/筛选/清空）依赖后端阶段六记忆抽取入库；真实对话后若面板为空且无「已记住」轻提示，为后端 aextract_facts 未产出（抽取 LLM 空返回，见 memory.md §13），待后端修复后自动生效。

### 第三波（前端阶段六 多智能体可视化 + 阶段七 长会话与策略生成体验，2026-08-25 完成）
- 深度分析 5 节点时间线/运行记录回看/策略校验状态/自动回测内嵌/模板库均已生效，无需新配置；端到端验证需后端配 DEEPSEEK_API_KEY + Celery backtest worker + 已同步标的数据。
- 运行记录「耗时」对非深度运行（普通对话/策略生成/失败）显示「--」：属后端 `agent_runs.duration_ms` 仅在深度模式 `_run_deep` 写入、`_run_react`/`_run_strategy`/失败/超时路径未写，前端 `total_duration` 已正确对接，待后端补写后自动显示。
